...
fn make_kernel(tex: texture_2d<f32>, sr: sampler, coord: vec2<f32>) -> array<vec4<f32>, 9> {
    let w = 1.0 / 50.;
    let h = 1.0 / 50.;

    var n: array<vec4<f32>,9>;
    n[0] = textureSample(tex, sr, coord + vec2(-w, -h));
    n[1] = textureSample(tex, sr, coord + vec2(0., -h));
    n[2] = textureSample(tex, sr, coord + vec2(w, -h));
    n[3] = textureSample(tex, sr, coord + vec2(-w, 0.));
    n[4] = textureSample(tex, sr, coord);
    n[5] = textureSample(tex, sr, coord + vec2(w, 0.));
    n[6] = textureSample(tex, sr, coord + vec2(-w, h));
    n[7] = textureSample(tex, sr, coord + vec2(0., h));
    n[8] = textureSample(tex, sr, coord + vec2(w, h));
    return n;
}
